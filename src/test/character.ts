...
describe('Characters', () => {
  ...
  describe('/GET characters/1000', () => {
    it('it should GET return an error for a character with id 1000', async () => {
      const { body } = await testEndpoint('/characters/1000');

      expect(body.error).to.equal('Error while retreiving data with id 1000.');
    });
  });
  ...
  describe('/GET characters?skip=5', () => {
    it('it should GET the sixth character', async () => {
  ...
  describe('/GET characters?sortBy=name&OrderBy=desc&limit=1', () => {
    it('it should GET the last character Zeke', async () => {
      const { body } = await testEndpoint('/characters?sortBy=name&OrderBy=desc&limit=1');
